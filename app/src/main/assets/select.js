...
var __ss={
	...
	__shows:function (){
		...
		if(this.IN_WAPCG==1||this.IN_WAPCG==2){$('s14').style.display="none";}
		...
		//if(this.IN_WAPCG==1){$('s14').style.display="none";}
		...
		if(this.IN_WAPCG==2){ //android

		}else if(this.IN_WAPCG==1){
			if(window.parent.frames["soonselectorder"].document.getElementById("sizixian")){		
				window.parent.frames["soonselectorder"].$("sizixian").checked=false;
			}
		...
		if(this.__classid==1){
			...
			if(this.IN_WAPCG==1||this.IN_WAPCG==2){$('s14').style.display="";}
			...
			$("s13").style.display="none";
			if(this.IN_WAPCG==1||this.IN_WAPCG==2){ if($('__peishu_qu').checked || $('__peishu_chu').checked)$('s14').style.display="";}
			/*$("psgd1").style.display="";
			$("psgd2").style.display="";
			$("psgd3").style.display="";
			$("psgd4").style.display="";
		...
		setTimeout("__ss.__danshuangset(1);",300);
		setTimeout("__ss.__danshuangset(2);",300);
	...
	__number_html:function(numberstr){ //全倒
		//if(this.IN_WAPCG==2){
		//	return numberstr+(this.__selectnumber!=""?"|":"");
		//}else{
		...
		if(n_arr!=''){
			...
			if($("__peishu_chu").checked||$("__peishu_chu2").checked){
				...
				for(i in pschu){
					...
					if(n_arr.toString().indexOf(pschu[i])==-1){
						set_arr[n]=pschu[i];
						n++;
					}
/*					if(stat==0){
...
*/				}
	...
	__dingwei_reg:'',
    __dingwei_shangjiang_reg:'',
    __dingwei_shangjiang_arr:new Array(4),
	__dingweitotal:0,
	__int_dingwei_num:function(){
		...
		this.__dingwei_shangjiang_reg="";
		this.__dingwei_shangjiang_arr=new Array(4);
		var sjstat= false;
		if(this.__shangjiang!='' && this.__dingwei_chu)sjstat=true;
		for( i = 0 ; i < this.__dingweiarray.length; i++ ){
		this.__dingwei_shangjiang_arr[i]="";
			if(this.__dingweiarray[i]!=''){
				...
				if(num.length>1){
					...
					for (j=0;j<num.length; j++)
					{
						...
						arrfuhao='0-9';
						comm="|";
						if(sjstat)getdata.splice(gnum,1,'');
					}
					if(sjstat){
						var comm2='',arrfuhao='';
						...
						this.__dingwei_shangjiang_arr[i]=arrfuhao+"";
				...
				this.__dingwei_shangjiang_reg += '['+arrfuhao+']';
				...
				if(this.__classid==1&&data.length==1||this.__classid==2&&data.length<=2||this.__classid==3&&data.length<=3){
					...
					this.__dingwei_exits=true;
					this.__dingwei_reg +='[0-9'+this.__fuhao+']';
					this.__dingwei_shangjiang_reg +='[0-9'+this.__fuhao+']';
				}else{
					this.__dingwei_reg += '['+data+this.__fuhao+']';
					this.__dingwei_shangjiang_reg += '['+data+this.__fuhao+']';
				}
				//this.__dingwei_reg += '['+data+this.__fuhao+']';
		...
		this.__dingwei_shangjiang_reg = new RegExp(this.__dingwei_shangjiang_reg);
	},
	__check_dingwei_reg:function(n){
		...
		if(this.__shangjiang!=''){
			if(this.__dingwei_chu){
				this.__dingweiarray=this.__dingwei_shangjiang_arr;
				console.log(this.__dingweiarray+'='+this.__dingwei_shangjiang_arr)
			}
			data = this.__shangjiang;
			...
			if(this.__classid==1){ //四字定号码重复排除，在对比 2017/3/11
			...
			}else if(this.__classid==1&&data.length==1||this.__classid==2){
				//document.writeln(n)
				if(data.length>=1){
            ...
            return ((this.__dingwei_chu)?(this.__dingwei_shangjiang_reg.test(nn)&&shangjiangreturn):this.__dingwei_reg.test(nn)&&shangjiangreturn);
		}else{
...
nnnn++;
	...
	__nnnum:0,
	__check_hanfushi_reg:function(n){ //只有复数除用
	...
	__int_danshuang:function(){//单数和双数初始化
		...
		for(var i=i_s;i<=4;i++){
			...
			if(d_checkeds==true){
				...
				//this.__dan_reg2 += '[13579'+this.__fuhao+']';

				//d_s=d_s+1;
				//this.__ds_num1=d_s;
			}else if(s_checkeds==true&&this.__shuang_qu){
				this.__dan_reg += '[02468'+this.__fuhao+']';
				//this.__dan_reg2 += '[02468'+this.__fuhao+']';
			}else{
			...
			if(s_checkeds==true){
				...
				//this.__shuang_reg2 += '[13579'+this.__fuhao+']';
				//s_s=s_s+1;
				//this.__ds_num2=s_s;
			}else if(d_checkeds==true&&this.__dan_qu){
				this.__shuang_reg += '[13579'+this.__fuhao+']';
				//this.__shuang_reg2 += '[13579'+this.__fuhao+']';
			}else{
...
//alert(this.__dan_reg +'=='+this.__shuang_reg)
	},
	__check_dan_reg:function(n){//认证单数 .认证双数
		if(!this.__dan_start && !this.__shuang_start)return true;
		var dan=true,shuang=true,danshuang=true;
		if(this.__dan_start)
		dan = this.__dan_reg.test(n);
		if(this.__shuang_start)
		shuang = this.__shuang_reg.test(n);
		danshuang = this.__danshuang_reg.test(n);
		if(this.__dan_chu && this.__shuang_qu){
			return (!dan && shuang);		
		}else if(this.__dan_qu && this.__shuang_chu){
			return (dan && !shuang);		
			...
			if(this.__classid == 1||this.__classid == 2){
				return (!(dan || shuang) && danshuang);
			}else{
				return !(dan || shuang);
			}
		}else if(this.__dan_qu&&!this.__shuang_qu){
			return (dan);
		}else if(!this.__dan_qu&&this.__shuang_qu){
			return (shuang);
		}else if(this.__dan_qu && this.__shuang_qu){
	...
	__create:function (){
		...
		if(this.IN_WAPCG==2){
			//alert(this.__selectnumber);
			$("selectnumber").value=this.__selectnumber;
			$("selectnumbertotal_hidden").value=this.__selectnumbertotal;
			$("selectlogs").value=this.__setselectlogs();
		}else{
	...
	__showdis:function(my,to){if($(to).checked)$(to).checked=false;this.__showpeishu(to);this.__danshuangchoqu(to);},
    ...
    __danshuangchoqu:function (s){
        if(s=='__dan_qu'||s=='__dan_chu'){
            __ss.__danshuangset(1);
        }else if(s=='__shuang_qu'||s=='__shuang_chu'){
    ...
    __danshuangset:function (s){
        ...
        if(s==2){
            ...
            namech ='双';
            $ss(name+"2").innerHTML='';
            if(!this.__shuang_chu&&!this.__shuang_qu)return ;
            if(this.__shuang_chu){
                str='除 ';
            }else{
                str='取 ';
            }
            ...
            $ss(name+"2").innerHTML='';
            if(!this.__dan_chu&&!this.__dan_qu)return ;
            if(this.__dan_chu){
                str='除 ';
            }else{
                str='取 ';
		...
		var hao='#';
		if(this.__classid==1||this.__classid==2||this.__classid==3){
			hao='X';
		}
        for (var i = num; i <=4; i++) {
            var danon=$ss('__'+name+'_'+i);
            if(danon.checked){
                str+=namech;
            }else{
                str+=hao;
    ...
    __danshuangshow:function (){
        for (var i = 1; i <=4; i++) {
            var danon=$ss('__dan_'+i);
            danon.onclick=function(){
                __ss.__danshuangset(1);
            };
            var shuangon=$ss('__shuang_'+i);
            shuangon.onclick=function(){
                __ss.__danshuangset(2);
            };
        }
